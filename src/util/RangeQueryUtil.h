...
public:
	...
	static bool fullRangeContainsEntry(const PHTree<DIM, WIDTH>& tree, const Entry<DIM, WIDTH> entryToCheck) {
		RangeQueryIterator<DIM, WIDTH>* it = getFullRangeIterator(tree);
		bool contained = rangeContainsEntry(it, tree, entryToCheck);
		delete it;
		return contained;
	}

	static bool rangeContainsEntry(const Entry<DIM, WIDTH> lowerLeft, const Entry<DIM, WIDTH> upperRight,
			const PHTree<DIM, WIDTH>& tree, const Entry<DIM, WIDTH> entryToCheck) {
	...
	static bool rangeContainsEntry(RangeQueryIterator<DIM, WIDTH>* it,
				const PHTree<DIM, WIDTH>& tree, const Entry<DIM, WIDTH> entryToCheck) {

			bool foundEqualEntry = false;
			while (it->hasNext() && !foundEqualEntry) {
				Entry<DIM, WIDTH> entryInRange = it->next();
				foundEqualEntry = entryInRange == entryToCheck;
			}
